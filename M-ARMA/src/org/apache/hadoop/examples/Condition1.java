...
public class Condition1  extends Configured implements Tool {
...
//		FileInputFormat.setMaxInputSplitSize(job, 10000);
	...
	// run program
	public static Float start(String[] args,int currentAttribute,int AttributeNum,float numberOfElements) throws Exception {
		...
		toBeUsedConditions.add(String.valueOf(currentAttribute));
		toBeUsedConditions.add(String.valueOf(AttributeNum+1));
		String condition1 = toBeUsedConditions.toString();
		
		FileSystem fileSystem = FileSystem.get(conf);
	    conf.set("num1", condition1);//条件属性（１,2,3...）,单个值
	    
		String numOfe = String.valueOf(numberOfElements);
		conf.set("NumofE", numOfe);
		
		int status1 = ToolRunner.run(conf, new Condition1(), args); 
