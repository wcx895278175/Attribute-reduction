...
import org.apache.hadoop.examples.Condition2.WordCountMap2;
import org.apache.hadoop.examples.Condition2.WordCountReduce2;
...
public class Reduction_con extends Configured implements Tool{
	...
	//step 1: map class
	 public static class WordCountMap extends Mapper<LongWritable, Text, Text,LongWritable>{
			...
			public void map(LongWritable mapInputKey,Text mapInputValue,Context context) throws IOException, InterruptedException{
				...
				String attr1 = temp.get("numreduc");
...
//				System.out.println("outkey:"+reduceOutputkey +"     outvalue:"+reduceOutputValue);
	...
	// run program
	public static float start(String[] args,ArrayList<Integer> conditions,float numberOfElements) throws Exception {
		...
		conf.set("numreduc", condition);
		String numOfe = String.valueOf(numberOfElements);
		conf.set("NumofE", numOfe);

		int status1 = ToolRunner.run(conf, new Reduction_con(), args);
		
		relyDegreeOfDToC = new Value2sum(fileSystem);
		relyDegreeOfDToC.read(fileSystem, args[2]);
