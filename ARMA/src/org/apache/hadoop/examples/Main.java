...
public class Main {
	...
	public static void init_ifo(int AttributeNum, String[] args ,float numberOfElements)  throws Exception {
		...
		for(int i = 1 ; i <= AttributeNum ; i++){
			...
			for(int j = i + 1 ; j <= AttributeNum ; j++){
				if(i != j ){
					new Condition2();
					importance_i_j[i][j] = importance_i_j[j][i] = Condition2.start(args,i,j, AttributeNum,numberOfElements);// 计算书属性i+j熵H（D|i+j）
		...
		for (int i = 1; i <= AttributeNum; i++) {
			for (int j = 1; j <= AttributeNum; j++) {
				...
				System.out.print(importance[i][j]+"("+i+","+j+")"+"    ");
			}
			System.out.println();
			dos.writeBytes("\n");
		}
	...
	public static float rely_condition(ArrayList<Integer> conditions,float numberOfElements,int AttributeNum)	throws Exception {
		...
		String[] arg_reduction = new String[] { "/2k_10/2k_10.txt","/2k_10/redu_condition_temp.txt","/2k_10/H_D_redu.txt" }; // 保存当前属性集合的等价类
		new Reduction_con();
	...
	public static void main(String[] args) throws Exception {
		...
		int AttributeNum = 10;// 条件属性个数
		float numberOfElements = 20000000f;// 样本总数
		int ant_Num = 1 ;// 蚂蚁数量
		int MAX_GEN = 1 ;//迭代次数
		// 初始化信息素矩阵，参数为条件属性个数
		String file_path_phe = "/2k_10/2k_10_phe.txt";// 生产信息度矩阵
		init_phe(AttributeNum,file_path_phe);

		// 初始化重要度矩阵，参数为条件属性个数+ 决策属性
		String[] file_path_ifo = new String[] { "/2k_10/2k_10.txt","/2k_10/temp_ifo.txt", "/2k_10/ifo.txt" ,"/2k_10/temp_ifo_D"};
	    ...
	    //这里以互信息作为依赖度
//		String[] arg_D = new String[] { "/2k_10/2k_10.txt","/2k_10/condition_D" };//决策属性的等价类	
//		float H_D = SigofC(arg_D,AttributeNum + 1 ,  numberOfElements);

		String[] arg_all = new String[] { "/2k_10/2k_10.txt","/2k_10/conditionAll_temp", "/2k_10/H_D_C" };// 分别表示文件路径，所有条件属性的等价类
...
//		System.out.println("Ｈ（D_C）:"+ H_D_C);
		System.out.println("4-4  2k_10  1*job  node * 2  antnum:1  gen= 1   程序运行时间： "+  (endTime-startTime)+"ms");
	}
